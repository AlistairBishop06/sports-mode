...
def fetch_speedway_scores():
    ...
    try:
        r = requests.get(SPEEDWAY_URL, headers=HEADERS, timeout=10)
        r.raise_for_status()
    except:
        return []
...
class SportsModeApp:
    ...
    def shutdown_pc(self):
        ...
        if messagebox.askyesno("Power Off", "Turn off the computer?"):
    ...
    def show_main_menu(self):
        ...
        football_btn = tk.Button(
            frame,
            text="Football",
            bg="#3182ce",
            command=self.show_football_leagues,
            **btn_style
        )

        football_btn.pack(pady=20)

        speedway_btn = tk.Button(
            frame,
            text="Speedway",
            bg="#d69e2e",
            command=lambda: self.show_scores("speedway"),
            **btn_style
        )
    ...
    def show_scores(self, mode, league=None):
        ...
        self.scores_box.pack(expand=True)
    ...
    def display_matches(self, matches):
        ...
        self.root.after(REFRESH_TIME, self.update_scores)
